...
// 비교하는 기린 사이 기린 값은(n-1)
// 마지막 스택을 확인하기 위해 마지막 값 뒤에 가장 큰값을 넣어줍니다.
function answer(giraffe) {
  ...
  for (let i = 0; i < giraffe.length; i++) {
    ...
    // 현재 기린 높이보다 낮으면 높이 계산 
    while (!stack.isEmpty() && stack.peek()["h"] < giraffe[i]) {    // "h" 기린 높이
      result += i - stack.pop()["i"] - 1; // "i" 기린의 인덱스
    }
