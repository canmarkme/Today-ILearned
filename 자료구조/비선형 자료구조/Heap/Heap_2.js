...
Heap.prototype.insert = function (item) {
  this.items[this.size()] = item;   // 트리의 가장 끝에 item을 넣어줍니다.
  this.bubbleUp();
};

// bubbleUp(): 추가된 노드 위치 정렬
Heap.prototype.bubbleUp = function () {
  let index = this.size() - 1;    // 마지막 index를 최초 시작으로 합니다.

  // 부모 여부 확인 && 자신보다 값을 비교하여 자신 > 부모라면 부모로 올라갑니다.
  while (this.parent(index) && this.parent(index) > this.items[index]) {
    this.swap(this.parentIndex(index), index);
    index = this.parentIndex(index);  // index 값도 업데이트
  }
};
