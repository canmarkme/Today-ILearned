...
## 6. text-align
텍스트의 수평 정렬. `text-align`은 블록 요소만 정렬할 수 있습니다. 인라인 요소는 width 프로퍼티가 없어 중앙 개념이 존재하지 않습니다. 
인라인 요소를 정렬하기 위한 방법(인라인 대표 예로 `<a>`태그)
1. `<a>` 태그 요소의 부모 요소로 블록 요소를 두고 `text-align` 프로퍼티를 사용하세요.
2. `<a>` 태그 요소에 `display: block;`을 지정한면 중앙 정렬이 가능합니다.
...
`text-decoration` 프로퍼티를 사용하여 `<a>` 태그 링크 underline을 제거할 수 있습니다.. 또는 텍스트에 underline, overline, line-through를 추가할 수도 있다.
