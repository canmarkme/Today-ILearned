...
- 만일 width와 height로 지정한 콘텐츠 영역보다 실제 콘텐츠가 크며 콘텐츠 영역을 넘치게 된다!!
...
### 1) max-width 
프로퍼티는 요소 너비의 최대값 지정, 프로퍼티는 요소 너비의 최소값 지정 합니다. 예를 들어 `max-width: 300px;` 의 경우 브라우저의 너비가 요소의 너비보다 좁아질 때 자동으로 요소의 너비가 줄어듭니다. 요소 너비가 브라우저 너비보다 크면 가로 스크롤바가 만들어지는데 이 문제를 해결하기 위해서 `max-width` 프로퍼티를 사용할 수 있습니다.
