...
<br>전체 선택자는 주의해서 쓰고 CSS 가장 상단에 쓸 것!!!
...
n은 0부터 시작하는 정수이다. 0과 음수는 생략되기 때문에 2n+1과 2n-1, 3n-2와 3n+1은 결과적으로 같은 수열을 생성한다. odd, even도 가능
...
### 1) 일반 형제 선택자 결합(General Sibling Combinator)
...
### 2) 인접 형제 셀렉터(Adjacent Sibling Combinator)

셀렉터 A의 형제 요소 중 셀렉터 A **바로 뒤**에 위치하는 셀렉터 B 요소를 선택합니다. A와 B 사이에 다른 요소가 존재하면 선택되지 않습니다.
